...
#[test]
fn probe_mod_invert_width_sweep() {
    use crypto_bigint::RandomBits;
    use rand_core::SeedableRng;

    for a_limbs in 1u32..=12 {
        for b_limbs in 1u32..=12 {
            let a_bits = a_limbs * Limb::BITS;
            let b_bits = b_limbs * Limb::BITS;
            let mut rng =
                chacha20::ChaCha8Rng::seed_from_u64(1000 + u64::from(a_limbs) * 100 + u64::from(b_limbs));
            let mut fails = 0;
            const N: u32 = 500;
            for trial in 0..N {
                let mut b = BoxedUint::random_bits(&mut rng, b_bits);
                b.set_bit_vartime(0, true);
                let b_odd = b.clone().to_odd().unwrap();
                let mut a = BoxedUint::random_bits(&mut rng, a_bits);
                if a.is_zero().into() {
                    a = BoxedUint::one_with_precision(a_bits);
                }

                let a_bi = to_biguint(&a);
                let b_bi = to_biguint(&b);
                let expected = a_bi.invm(&b_bi);
                let actual = a.invert_odd_mod(&b_odd).into_option();

                let ok = match (&expected, &actual) {
                    (Some(exp), Some(act)) => *exp == to_biguint(act),
                    (None, None) => true,
                    _ => false,
                };
                if !ok {
                    ...
                    if fails <= 2 {
                        eprintln!(
                            "FAIL a_limbs={a_limbs} b_limbs={b_limbs} trial={trial} a={a:#x} b={b:#x} expected={expected:?} actual={actual:?}"
                        );
                    }
                }
            }
            if fails > 0 {
                eprintln!("a_limbs={a_limbs} b_limbs={b_limbs}: {fails}/{N} failed");
            }
        }
    }
}

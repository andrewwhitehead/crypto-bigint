...
pub const fn partial_xgcd<const HALTING: bool>(
    ...
    while i < steps {
        ...
        let apply_sub = a_odd.and(unhalted);

        let (hi_diff, hi_borrow) = a_hi.overflowing_sub(b_hi);
        let swap_raw = Choice::from_u8_lsb(hi_borrow as u8);
        let apply_swap = swap_raw.and(apply_sub);
        let lo_diff = a_lo.wrapping_sub(b_lo);
        let abs_lo_diff = word::select(lo_diff, lo_diff.wrapping_neg(), swap_raw);
        let abs_hi_diff = word::select(hi_diff, hi_diff.wrapping_neg(), swap_raw);
        ...
        (a_lo, a_hi, b_lo, b_hi) = (
            word::select(a_lo, abs_lo_diff, apply_sub) >> 1,
            word::select(a_hi, abs_hi_diff, apply_sub) >> 1,
...
#[inline(always)]
#[must_use]
pub const fn partial_xgcd_word(mut a: Word, mut b: Word, steps: u32) -> (BingcdMatrix, Word) {
    debug_assert!(b & 1 == 1, "b must be odd");
    ...
    while i < steps {
        ...
        let swap_raw = Choice::from_u8_lsb(borrow as u8);
        let apply_swap = swap_raw.and(apply_sub);
        let abs_diff = word::select(diff, diff.wrapping_neg(), swap_raw);

        (a, b) = (
            word::select(a, abs_diff, apply_sub) >> 1,
...
pub const fn partial_xgcd_vartime(
    ...
    loop {
        ...
        let (hi_diff, swap) = a.1.overflowing_sub(b.1);
        let swap_mask = (swap as Word).wrapping_neg();
        let abs_hi_diff = (hi_diff ^ swap_mask).wrapping_sub(swap_mask);
        let a_b = a.0 & b.0;
        jacobi_neg ^= swap_mask & a_b & (a_b >> 1);
...
mod tests {
    ...
    fn partial_xgcd_matches_elementary_steps() {
        use crate::{Choice, Limb, Random, Uint, Word, modular::gcd::GCD_BATCH_SIZE};

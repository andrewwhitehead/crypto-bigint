...
impl<'a> CofactorPair<'a> {
    ...
    /// Conditionally apply any pending modular division by `2^k` to both `u` and `v`.
    ///
    ...
    /// Applies one round's matrix `m` to `(u, v)`, growing the tracked window first if the
    /// running bit budget says this round's own worst-case growth won't fit and there's still
    /// room to grow into, then reducing by `2^k mod y` whenever the window is full,
    ...
    /// Vartime equivalent of [`Self::apply_matrix`]: applies the matrix, then grows the window by
    /// one limb only if `(u, v)` actually overflowed it this round -- measured directly from
    /// their own data via [`Self::overflow_vartime`], rather than guessed from a schedule.
    #[inline(always)]
    ...
    /// A no-op if the window is already full and nothing is pending.
    #[inline(always)]
    ...
    /// Vartime equivalent of [`Self::flush`].
    #[inline(always)]
    pub const fn flush_vartime(&mut self) {
        self.grow_to(self.u.nlimbs());
        self.reduce_k_vartime();
    }
    ...
    /// Finishes tracking: flushes (grows to full width and reduces any pending `k`),
    /// reduces `v` into `[0, y)`, and drops the `hi` extension.
    ///
    ...
    /// Vartime equivalent of [`Self::finalize`]: flushes, then reduces `v` into `[0, y)`.

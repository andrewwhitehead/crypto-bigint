...
use crate::{Choice, Limb, NonZero, Odd, UintRef, Word, word};
...
impl ExtendedIntRef<'_> {
    ...
    /// The absolute value of `self`'s own lowest limb.
    #[inline(always)]
    pub const fn abs_low_limb(&self) -> Word {
        let raw = self.lo.limbs[0].0;
        word::select(raw, raw.wrapping_neg(), self.is_negative())
    }

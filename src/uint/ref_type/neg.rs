...
impl UintRef {
    /// Perform wrapping negation: map `self` to `(self ^ 1111...1111) + 0000...0001`.
    /// Return whether the carry flag is set on the addition.
    ///
    /// Note: the carry is set if and only if `self` is zero.
    #[inline]
    pub(crate) const fn carrying_neg_assign(&mut self) -> Limb {
    ...
    /// Returns the carry limb.
    ///
    /// Note: the carry limb is non-zero if and only if `self` is zero.
    #[inline]
    pub(crate) const fn conditional_carrying_neg_assign(&mut self, apply: Choice) -> Limb {
        let mut carry = Limb::ONE;
        let mut i = 0;
        while i < self.limbs.len() {
            let neg;

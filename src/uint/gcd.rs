...
use crate::{
    Choice, Gcd, Int, Limb, NonZero, NonZeroUint, Odd, OddUint, Uint, UintRef, Xgcd, modular::gcd,
    primitives::u32_min,
};
...
impl<const LIMBS: usize> OddUint<LIMBS> {
    ...
    pub const fn gcd_unsigned(&self, rhs: &Uint<LIMBS>) -> Self {
        ...
        if const { LIMBS <= gcd::SMALL_THRESHOLD_LIMBS } {
            gcd::gcd_odd_small(f.as_mut_uint_ref(), g.as_mut_uint_ref());
        } else {
            gcd::gcd_odd(f.as_mut_uint_ref(), g.as_mut_uint_ref());
        }
